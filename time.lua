-------------------------------------------------------
-------------------------
-- A library for the manipulation of dates and periods 
according to the
-- Gregorian calendar.
--
-- Credit: the Gregorian calendar routines contained in 
this library are 
-- ported from Claus Tøndering calendar algorithms:
-- http://www.tondering.dk/main/index.php/calendar-information .
--
-- Copyright (C) 2011-2016 Stefano Peluchetti. All 
rights reserved.
-------------------------------------------------------
-------------------------
...
local type, new, istype, tonumber = type, ffi.new, 
ffi.istype, tonumber
...
local function p_64(ticks)
   ...
   local p_mt = {
    ...
    return new(ct, 
   h*(1e6*60*60LL)+m*(1e6*60LL)+s*(1e6*1LL)+ms)
    end,
    copy = function(self)
    ...
    __add = function(self, rhs) T_same(self, rhs) --
        Commutative.
       ...
       self:seconds(), self:microseconds()
        ...
        tominutes = function(self)
           ...
           local function weeks(x) T_int(x) return 
           p_64(x*(1e6*60*60*24*7LL)) end
           local function days(x) T_int(x) return 
           p_64(x*(1e6*60*60*24LL)) end
           local function hours(x) T_int(x) return 
           p_64(x*(1e6*60*60LL)) end
           local function minutes(x) T_int(x) return 
           p_64(x*(1e6*60LL)) end
           local function seconds(x) T_int(x) return 
           p_64(x*(1e6*1LL)) end
           local function milliseconds(x) T_int(x) return 
           p_64(x*(1e3*1LL)) end
           local function microseconds(x) T_int(x) return 
           p_64(x*1LL) end
           local function toperiod(x)
            if type(x) == "string" then
            local f1, l1, h, m, s, ms = 
           x:find("(%d+):(%d+):(%d+).(%d+)")
            if h == nil or ms == nil or l1 ~= #x then
            error("'"..x.."' is not a string representation 
           of a period")
            end
            local ton = tonumber
           ...
           local months_ct = ffi.metatype("struct { int32_t _m; 
           }", months_mt)
           -- Years ----------------------------------------------
           -------------------------
           local years_mt = {
           ...
           local years_ct = ffi.metatype("struct { int32_t _y; }", 
           years_mt)
           -- Date -----------------------------------------------
           -------------------------
           -- It's date(1582, 1, 1):
...
local function T_date_range(ticks)
 if not (d_ticks_min <= ticks and ticks <= 
d_ticks_max) then
 error("resulting date is outside the allowed 
range")
 end
end
...
local function T_year(year) T_int(year)
    ...
    error("year "..year.." outside the allowed range 
   [1582, 9999]")
    end
   end
   local function T_month(month) T_int(month)
    if not (1 <= month and month <= 12) then
    error("month "..month.." outside the allowed range 
   [1, 12]")
    end
   end
   ...
   local eom = { 31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 
   30, 31 }
   local function endofmonth(year, month) T_year(year) 
   T_month(month)
    return (month == 2 and isleapyear(year)) and 29 or 
   eom[month]
   end
   local function T_day(year, month, day)
    if not (1 <= day and day <= endofmonth(year, month)) 
   then
    error(year.."-"..month.."-"..day.." is not a valid 
    date")
     end
    end
    local function weekday(year, month, day) T_year(year) 
    T_month(month) 
     T_day(year, month, day)
     local a = floor((14 - month)/12)
     local y = year - a
     local m = month + 12*a - 2
     local d = (day + y + floor(y/4) - floor(y/100) + 
    floor(y/400) +
     floor((31*m)/12)) % 7
     return d == 0 and 7 or d -- Days of week from 1 = 
    Monday to 7 = Sunday.
    end
...
-- Assumes only violation of valid date may be in day 
outside of end of month
-- due to months and years shifts. Cap the day and 
...
local d_mt = {
 __new = function(ct, year, month, day) T_year(year) 
T_month(month) 
 T_day(year, month, day)
 return new(ct, ymd_to_julian(year, month, 
day)*(86400LL*1e6))
end,
copy = function(self)
...
return valid_date_cap_day(year, month, day) + 
d:period()
...
self:period()
 ...
 return format("%i-%02i-%02iT", year, month, 
day)..posptostr(h, m, s, ms)
 end,
 ticks = p_mt.ticks,
 ymd = function(self)
 ...
 year = function(self) local y, m, d = self:ymd() 
return y end,
 month = function(self) local y, m, d = self:ymd() 
return m end,
 day = function(self) local y, m, d = self:ymd() 
return d end,
 ...
 isleapyear = function(self) 
   ...
   if jit.os == "Windows" then -- On Windows sizeof(long) 
   == 4 on both x86 and x64.
    ffi.cdef[[
    ...
    struct {
        ...
        nowlocal = function()
            C.GetLocalTime(st) 
            return d_ct(st.wYear, st.wMonth, st.wDay) + 
           p_ct(st.wHour, st.wMinute,
            st.wSecond, st.wMilliseconds*1000)
            end
            local epoch_offset = d_ct(1601, 1, 1):ticks()
            local function if_available(lib, fname)
            local ok, f = pcall(function() return lib[fname] 
           end)
            return ok and f
            end
            local get_system_time = if_available(C, 
           'GetSystemTimeAsPreciseFileTime')
            or C.GetSystemTimeAsFileTime
            nowutc = function()
            -- Resolution: 1 microsecond.
            -- Accuracy : 1 millisecond up to Windows 7, 
           higher otherwise.
            get_system_time(ft)
       ...
       need higher resolution.
        ...
        return d_ct(1900 + tm.tm_year, 1 + tm.tm_mon, 
       tm.tm_mday) +
        p_ct(tm.tm_hour, tm.tm_min, tm.tm_sec, 
       tv[0].tv_usec)
        end
        nowutc = function()
        -- Resolution: 1 microsecond.
        ...
        C.gettimeofday(tv, nil)
        return new(d_ct, epoch_offset + 
       tv[0].tv_sec*1000000LL + tv[0].tv_usec)
        end
        sleep = function(p)
            if p < p_ct() then
            error("cannot sleep a negative amount of time")
