...
park_tube = queue.create_tube("park_tube", "fifottl", { 
if_not_exists = true})
-- Parking Function 
...
function check_vehicle(v)
 ...
 return base
   ...
   function create_ticket(vehicle)
    ...
    return park_tube:put({ type = 1, ticket_id = 
   ticket_id, time_check_in = time_in, base_price = price 
   }, {ttl = 60})
   end
   -- D. Remote Call: check_in() function
   function check_in(ticket_id, time_check_in, base_price)
    -- INSERT data into SQL
    sql_statement = "INSERT INTO ParkData (ticket_id, 
   check_in, base_price) VALUES ('".. ticket_id .."','".. 
   time_check_in .."', ".. base_price .. ");"
    box.execute(sql_statement)
   end
   -- E. Function to select time_in
   function select_time_in(ticket_id)
    select_statement = "SELECT check_in FROM ParkData 
...
function select_bprice(ticket_id)
 select_statement = "SELECT base_price FROM ParkData 
WHERE ticket_id='".. ticket_id .."';"
 data_in = box.execute(select_statement)
...
-- H. Function to select time from ticket id and clean 
it with python also as check out request
function clean_time_string(ticket_id)
 ...
 -- Put task to call check out function with cleaned 
time 
-- Send to python to clean unnecessary quote to 
also served as check out request - type 2 
 return park_tube:put({ type = 2, ticket_id = 
ticket_id, text = clean_time_in }, {ttl = 60})
end
...
function check_out(ticket_id, time_check_in) 
 ...
 local duration_to_minutes = (duration_hours * 60) + 
duration_minutes
 print("Total duration in minutes:" .. 
duration_to_minutes)
 -- Calculate total payment
 local calculation_total = duration_to_minutes / 60 
* base_price
 -- Round total payment
 local calculation_total = 
math.floor(calculation_total) 
 print("Total payment: " .. calculation_total)
 -- Update SQL
 update_statement = "UPDATE ParkData SET 
 check_out='".. time_out_str .."', duration=".. 
 duration_to_minutes ..", total_payment=".. 
 calculation_total .." WHERE ticket_id='".. ticket_id 
 .."';"
  box.execute(update_statement)
  -- Put task
  return park_tube:put({ type = 3, ticket_id = 
 ticket_id, total_payment = calculation_total }, { ttl = 
 60 })
 end 
 ...
 function save_payment_id(ticket_id, trx_id, qr_link)
  ...
  -- Update SQL
  update_statement = "UPDATE ParkData SET 
 payment_id='".. trx_id .. "' WHERE ticket_id='".. 
 ticket_id .. "';"
  box.execute(update_statement)
 end
 -- K. Confirm Payment
 function confirm_pay(order_id)
  return park_tube:put({ type = 4, order_id = 
 order_id }, { ttl = 60 })
 end